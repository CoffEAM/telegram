...
async def main_callback(query: types.CallbackQuery, state: FSMContext):
    ...
    elif clicked_button in ['5', '6', '7', '8', '9']:
...
async def day_callback(query: types.CallbackQuery, state: FSMContext):
    clicked_button = query.data.split("_")[-1]
    print(clicked_button)
    if clicked_button in ['пн', 'вт', 'ср', 'чт', 'пт', 'сб']:
...
async def print_lessons(callback: types.CallbackQuery, state: FSMContext):
    ...
    answer = f'<b>{data["class_num"]} класс {str(data["profile_id"]).title()}, {data["day"]}</b>' + "\n"
    for lesson in range(len(lessons)):
        if lessons[lesson] not in ['Разговоры о важном', 'Образовательные модули', 'Россия - мои горизонты', 'Нет урока']:
            answer += f'  <b>{nums[lesson][0]}.</b> <i>{times[lesson]}</i> — {lessons[lesson]}'+"\n"
        elif lessons[lesson] in ['Разговоры о важном', 'Образовательные модули', 'Россия - мои горизонты']:
            answer += f'  <b>{nums[lesson][0]}</b> <i>{times[lesson]}</i> — <u>{lessons[lesson]}</u>' + "\n"
...
async def updated_inline_profiles(callback: types.CallbackQuery, class_name: int, state: FSMContext):
    ...
    inline_kb2.button(text=f"Назад◀️", callback_data=f"symbol_back")
